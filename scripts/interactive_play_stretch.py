...
Description: Manually control the stretch robot to interact with the environment. 

Adapted from https://github.com/facebookresearch/habitat-lab/blob/main/examples/interactive_play.py

Run as
```
python examples/interactive_play_stretch.py
```
...
- For velocity control
    - 1-7 to increase the motor target for the stretch robot arm joints
    - Q-U to decrease the motor target for the stretch robot arm joints
- For IK control
    - W,S,A,D to move side to side
    - E,Q to move up and down
- I,J,K,L to move the stretch robot base around
- PERIOD to print the current world coordinates of the stretch robot base.
...
- X to change the agent that is being controlled (if there are multiple agents).
...
python examples/interactive_play.py --save-obs --save-obs-fname "/path/to/habitat/play"
...
def get_input_vel_ctlr(
    ...
    if key == ord("."):
        # Print the current position of the stretch robot, useful for debugging.
        pos = [
    ...
    if key == ord(","):
        # Print the current arm state of the stretch robot, useful for debugging.
        joint_state = [
    ...
    print(
        "base_action_name in env.action_space.spaces",
        base_action_name in env.action_space.spaces,
    )
...
def save_current_frames(obs, save_obs_keys, base_dir=SAVE_VIDEO_DIR):
    """
    Save the current observation frames to a timestamped directory
    """
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    save_dir = osp.join(base_dir, f"frames_{timestamp}")
    os.makedirs(save_dir, exist_ok=True)
    
    for key in save_obs_keys:
        if key in obs:
            frame = obs[key]
            if frame is not None:
                # Save based on observation type
                if "rgb" in key:
                    # Save RGB image
                    cv2.imwrite(osp.join(save_dir, f"{key}.png"), frame[..., ::-1])  # Convert RGB to BGR
                elif "depth" in key:
                    # Normalize and save depth image
                    depth_norm = (frame / np.max(frame) * 255).astype(np.uint8)
                    cv2.imwrite(osp.join(save_dir, f"{key}.png"), depth_norm)
                elif "top_down_map" in key:
                    # Save top-down map
                    cv2.imwrite(osp.join(save_dir, f"{key}.png"), frame)
                else:
                    # For other observation types
                    np.save(osp.join(save_dir, f"{key}.npy"), frame)
    
    print(f"Saved current frames to {save_dir}")
    return save_dir
...
def play_env(env, args, config):
    ...
    obs = env.reset()
    # print(config.habitat.simulator.agents.main_agent.sim_sensors)
    # print("Observation keys: ", obs.keys())
    # print("Observation shapes: ", {k: obs[k].shape for k in obs.keys()})
    # exit()
    ...
    # Check if top-down map is enabled in the measurements
    top_down_map_measure = env.task.measurements.measures.get("top_down_map", None)
    ...
    while True:
        ...
        if not args.no_render and key == ord("f"):
            # Save the current frame when 'f' key is pressed
            save_current_frames(obs, args.save_obs_keys)
        ...
        # print(obs.keys())
        if args.save_obs:
            # Extract top-down map from measurements if available
            if top_down_map_measure is not None and "top_down_map" in info:
                top_down_map = info["top_down_map"]["map"]
                obs["top_down_map"] = top_down_map
            
            selected_obs = {key: obs[key] for key in args.save_obs_keys if key in obs}
            for key, value in selected_obs.items():
                # print(f"Saving {key} observation")
                # print(value.shape)
    ...
    if args.save_obs:
        ...
        for key, frames in all_obs.items():
            frames = np.array(frames)
            if "rgb" in key:
                # For RGB frames
                vut.make_video(
                    np.expand_dims(frames, 1),
                    0,
                    "color",
                    osp.join(SAVE_VIDEO_DIR, f"{args.save_obs_fname}_{key}.mp4"),
                )
            elif "depth" in key or "top_down_map" in key:
                # For depth frames and top-down map (single channel)
                frames = frames.squeeze()  # Assuming depth frames are single channel
                vut.make_video(
                    np.expand_dims(frames, 1),
                    0,
                    "depth",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-obs-fname", type=str, default="play")
    parser.add_argument(
        "--save-obs-keys",
        type=str,
        nargs="+",
        default=["head_rgb", "head_depth", "third_rgb"],
        help="List of observation keys to save.",
    )
    parser.add_argument(
        "--enable-top-down-map",
        action="store_true",
        default=False,
        help="Include top-down map in observations.",
    )
    ...
    # Add top-down map to save_obs_keys if requested
    if args.enable_top_down_map and "top_down_map" not in args.save_obs_keys:
        args.save_obs_keys.append("top_down_map")

    config = habitat.get_config(args.cfg, args.opts)
    
    # Ensure top_down_map measurement is enabled if it's included in save_obs_keys
    with habitat.config.read_write(config):
        if (args.enable_top_down_map or "top_down_map" in args.save_obs_keys) and "top_down_map" not in config.habitat.task.measurements:
            config.habitat.task.measurements.update(
                {"top_down_map": TopDownMapMeasurementConfig()}
            )

    # config = config.copy()
    data_config = config.habitat.dataset
    # data_config.data_path = "./data/datasets/replica_cad/rearrange/v1/train/rearrange_easy.json.gz"
    with habitat.config.read_write(
        data_config
    ):  # Explicitly set data_config to writable
        data_config.data_path = (
            "./data/datasets/replica_cad/rearrange/v2/train/rearrange_easy.json.gz"
            # "./data/datasets/replica_cad/rearrange/v2/val/rearrange_easy.json.gz"
            # "./data/datasets/replica_cad/rearrange/v2/train/all_receptacles_10k_1k.json.gz"
        )
